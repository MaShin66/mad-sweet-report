...
`stuck`(감시 작동 안 함)과 `recovered`(감시 정상화)는 상품이 아니라 감시기 자체에 대한
알림이라 `notifyOn` 과 무관하게 항상 나간다. `alertStuckAfter: 0` 으로 끌 수 있다.
...
먼저 이걸 돌린다. 설정 → 요청 → 판정 → 상태 → 알림 판단까지 전부 보여준다.
실제로 알림을 보내지는 않는다.

```bash
node watch.js diagnose
```

원인은 대개 이 다섯 중 하나다.

| 원인 | diagnose 에서 보이는 것 | 대응 |
|---|---|---|
| 감시기가 안 돌고 있었다 | `기록: 없음` 또는 오래된 시각 | `ps`, cron 로그 확인 |
| 주기가 길어 재고 창을 놓쳤다 | `검사 주기: 300초` | 60~120초로 줄인다 |
| 쿠팡이 막았다 | `응답: HTTP 403` | `--browser`, 주기 늘리기, 가정용 회선 |
| 재고 판정 실패 | `판정: 판정불가` | `dump` 로 마크업 확인 |
| 알림 채널 설정 누락 | `알림 채널: ... !! 비어 있음` | 환경변수 확인 |

### 주기를 얼마로 둘 것인가

들어오자마자 빠지는 품목이면 300초는 길다. 재고가 2분만 열렸다 닫히면
5분 주기로는 아예 못 본다. 다만 짧을수록 차단이 빨라진다.

- 하루에도 몇 번 들어왔다 빠지는 인기 품목: **60~120초**
- 며칠에 한 번 들어오는 품목: **300~600초**

`403` 이 뜨기 시작하면 주기를 늘리거나 `--browser` 로 바꾼다.

### 감시기가 조용히 죽는 것 자체를 알린다

가장 나쁜 실패는 감시기가 멈췄는데 사용자는 "아직 품절인가 보다" 하고
넘어가는 것이다. 재고 판정이 `alertStuckAfter` 회(기본 3회) 연속 실패하면
**그 사실을 알림 채널로 보낸다.** 다시 정상으로 돌아오면 정상화 알림도 간다.

```jsonc
"alertStuckAfter": 3,        // 0 이면 끔
"stuckCooldownMinutes": 360  // 장애 알림이 6시간에 한 번을 넘지 않게
```

기록을 남겨두면 나중에 원인을 따지기 쉽다.
...
페이지 전체를 훑으면 안 된다. 쿠팡 상품 페이지에는 "함께 본 상품", "다른 옵션",
리뷰가 같이 실리고 **그중 품절된 게 하나라도 있으면 "일시품절" 이 잡힌다.**
본품이 멀쩡히 팔리는데 품절로 읽히는 것이다. 그래서 세 단계로 나눈다.

1. **JSON-LD** (`application/ld+json` 의 `offers.availability`) — 본품을 특정해서
   서술하므로 가장 믿을 만하다. 있으면 이것만 본다. Product 노드가 여러 개면
   문서에 먼저 나오는 것을 본품으로 잡는다.
2. **og 메타** (`product:availability`) — 그다음.
3. **텍스트 추론** — 위 둘이 없을 때만. 그것도 구매 영역(`prod-atf`, `prod-buy` …)
   으로 범위를 좁혀서 본다. 좁혔더니 아무 신호도 없으면 잘라낸 위치가 틀린
   것이므로 문서 전체로 돌아간다.

3단계에서 판매중/품절 신호 세기 차이가 15 미만이면 판정을 포기하고
알림을 보내지 않는다. `diagnose` 의 `경로` 줄에서 어느 단계로 판정했는지 볼 수 있다.
